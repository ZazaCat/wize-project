...
def handle_file_upload():
    ...
    allowed_models_with_images = {"gpt-4", "gpt-4-turbo", "gpt-4-turbo-2024-04-09"}
    selected_model = st.session_state.selected_model

    uploaded_files = st.session_state.file_uploader
    if uploaded_files:
        if selected_model not in allowed_models_with_images:
            st.warning("Image uploads are only supported by gpt-4, gpt-4-turbo, and gpt-4-turbo-2024-04-09 models.", icon="⚠️")
            return

        st.session_state.is_processing = True
        if all(file.size <= 5 * 1024 * 1024 for file in uploaded_files):  # Check if all file sizes are within the limit
            for uploaded_file in uploaded_files:
                ...
                if any(filename.endswith(ext) for ext in valid_text_extensions):
                    ...
                    mime_type = Image.open(io.BytesIO(image_bytes)).format.lower()
                    image_base64 = f"data:image/{mime_type};base64,{encoded_image}"  # Use MIME type correctly
                    if "image_bundles" not in st.session_state:
                        st.session_state.image_bundles = []
                    st.session_state.image_bundles.append(image_base64)  # Collect image base64 strings
                    ...
                    if len(st.session_state.image_bundles) == 4 or len(uploaded_files) == st.session_state.file_uploader.index(uploaded_file) + 1:
                        st.session_state.conversations[st.session_state.current_conversation].append({
                            ...
                            "images": st.session_state.image_bundles  # Store the base64 strings for JSON serialization
                        })
                        st.session_state.image_bundles = []  # Reset the bundle
                        st.session_state.all_conversations[st.session_state.username] = st.session_state.conversations
            ...
            st.error("The maximum file size allowed is 5MB each.")
...
if st.session_state.username is None:
    ...
    with st.sidebar:
        ...
        st.file_uploader("Upload files (text or images, max 4)", type=[
            ...
            'rb', 'swift', 'kt', 'kts', 'rs', 'jpg', 'jpeg', 'png', 'webp'
