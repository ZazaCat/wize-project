...
def display_chat(conversation_name):
    for message in st.session_state.conversations.get(conversation_name, []):
        ...
        with st.chat_message(message["role"], avatar=avatar_url):
            ...
            # Add video handling here
            elif "videos" in message:
                for video_base64 in message["videos"]:
...
def extract_frames_from_video(video_bytes, interval=1):
    frames = []
    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            # Adding video processing
                ...
                frames = extract_frames_from_video(file_contents)
                send_frames_to_omniplex(frames)

                # Store as conversation messages
                video_encoded = base64.b64encode(file_contents).decode("utf-8")
                video_base64 = f"data:video/{filename.split('.')[-1]};base64,{video_encoded}"

                st.session_state.conversations[st.session_state.current_conversation].append({
                    "role": "user",
                    "content": f"Video content of {uploaded_file.name}",
                    "videos": [video_base64]
                })
                ...
                st.toast(f"Unsupported file type for {uploaded_file.name}. Only text, image, and video files are allowed.", icon="❌")
                st.session_state.is_processing = False
...
def main_ui():
    if st.session_state.username is None:
        ...
        with st.sidebar:
            ...
            # File upload
            st.file_uploader("Upload files (text, image, or video, max 4)", type=[
                "py", "txt", "json", "js", "css", "html", "xml", "csv", "tsv", "yaml", "yml",
                "ini", "md", "log", "bat", "sh", "java", "c", "cpp", "h", "hpp", "cs", "go",
                "rb", "swift", "kt", "kts", "rs", "jpg", "jpeg", "png", "pdf", "docx", "mp4", "avi", "mov", "mkv"
