...
def display_chat(conversation_name):
    for message in st.session_state.conversations.get(conversation_name, []):
        avatar_url = ":material/person:" if message["role"] == "user" else "https://cdn.discordapp.com/attachments/1219689426750410793/1245979698207391755/20240531_135821.jpg?ex=665ab885&is=66596705&hm=73123368e8de2d3d424283af675b75ca863a0657b590c2c5232324be2f6eaea2&"
...
Output: install nvidia graphics drivers ubuntu 22.04\nubuntu 22.04 nvidia driver installation"""
    ...
    payload = {
        "messages": [{"role": "system", "content": prompt}] + chat_history + [{"role": "user", "content": "Generate a search query based on this input, do not engage in a conversation, no commentary, you are not a conversational chatbot or an assistant. Do not say anything else, or respond with other messages other than the search query, here's my input: " + query}],
        "model": "gpt-4o",
        "temperature": 0.7,
        "max_tokens": 4096,
        "top_p": 1,
...
def scrape_and_process_results(queries, max_results_per_query):
    ...
    if num_queries == 3:
        max_results_per_query = 1  # Top 1 result for each query if there are 3 queries
    elif num_queries == 2:
        max_results_per_query = 2  # Top 2 results for each query if there are 2 queries
    else:
        max_results_per_query = 3  # Top 3 results for single query
    ...
    if st.session_state.username is None:
        ...
        if current_conversation:
            ...
            if st.session_state.is_processing:
                ...
                with st.chat_message("assistant", avatar="https://cdn.discordapp.com/attachments/1219689426750410793/1245947832939057153/20240531_115141.jpg?ex=665a9ad7&is=66594957&hm=c3c1267734a972de789a38de63ea98915e0c8daadf9e04621809ab7c765ae767&"):
